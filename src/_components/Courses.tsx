...
const ReadingCard = ({ readingId }: { readingId: string }) => (
  <Card
    className="bg-green-100 rounded-lg p-4 mt-4"
    padding="lg"
    radius="md"
    withBorder
    style={{ flex: 1 }}
  >
    <Text className="text-md font-bold" color="green">
      Reading
    </Text>
    <Button
      component="a"
...
const HomeworkCard = () => (
  <Card
    className="bg-yellow-100 rounded-lg p-4 mt-4"
    padding="lg"
    radius="md"
    withBorder
    style={{ flex: 1 }}
  >
    <Text className="text-md font-bold" color="yellow">
      Homework
    </Text>
    <Button
      component="a"
      href="/homeworks/test-homework"
      color="yellow"
      variant="light"
      fullWidth
      mt="sm"
...
export default function Courses() {
  ...
  } = api.announcements.findAll.useQuery();
  const {
    data: apiClasses,
    isLoading: classesLoading,
    error: classesError,
  } = api.classes.findAll.useQuery();
  const {
    data: apiUnits,
    isLoading: unitsLoading,
    error: unitsError,
  } = api.units.findAll.useQuery();

  const announcements: Announcement[] =
    apiAnnouncements?.map(transformAnnouncement) || [];
  ...
  return (
    <MantineProvider>
      <Container
        className="flex items-center w-full min-h-screen py-6"
        style={{ maxWidth: "1400px", fontFamily: "'Muli', sans-serif" }}
      >
        <div className="container mx-auto flex flex-wrap items-start">
          <div className="w-full mb-6">
            <Card
              className="bg-blue-100 rounded-lg p-6"
              padding="lg"
              radius="md"
              withBorder
              style={{ width: "100%" }}
            >
              <Text className="text-xl font-bold mb-4" color="blue">
                Announcements
              </Text>
              ...
              {classesLoading ? (
                ...
                classes.map((classItem) => (
                  <div key={classItem.id} className="p-4 w-full">
                    <Text className="text-xl font-bold mb-4 text-neutral-600">
                      {classItem.name}
                    </Text>
                    <Card
                      className="bg-neutral-100 rounded-lg w-full shadow-lg p-6 relative mb-6"
                      padding="lg"
                      radius="md"
                      withBorder
                    >
                        ...
                        {unitsLoading ? (
                          ...
                          units
                            ...
                            .map((unit) => (
                              <Card
                                key={unit.id}
                                className="bg-white rounded-lg w-full transform hover:translate-y-1 hover:shadow-xl transition duration-300 mb-4"
                                shadow="sm"
                                padding="lg"
                                radius="md"
                                withBorder
